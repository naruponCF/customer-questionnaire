...
export function renderAdminDashboard(
  ...
  const visibleSubs = isAdmin
    ? submissions
    : submissions.filter((s) => s.distributor === distributor);

  const tableRows = visibleSubs
    .map(
      (s) => `<tr data-id="${s.id}">
        ...
        <td>
          ${isAdmin
            ? `<select class="distributor-select" data-id="${s.id}">
                <option value=""${!s.distributor ? " selected" : ""}>— Unassigned —</option>
                <option value="SoftDebut"${s.distributor === "SoftDebut" ? " selected" : ""}>SoftDebut</option>
                <option value="Nforce"${s.distributor === "Nforce" ? " selected" : ""}>Nforce</option>
              </select>`
        ...
        <td class="actions">
          ...
          ${isAdmin ? `<button class="delete-btn" data-id="${s.id}">Delete</button>` : ""}
        </td>
      </tr>`,
    )
    .join("");
...
<head>
  ...
  <style>
    * { margin:0; padding:0; box-sizing:border-box; }
    ...
    .view-btn, .delete-btn { padding:0.3rem 0.7rem; border:none; border-radius:5px; font-size:0.8rem; cursor:pointer; font-weight:600; }
    .view-btn { background:#F38020; color:#fff; }
    .view-btn:hover { background:#e06d1a; }
    ...
    .badge-soft { background:#e8f5e9; color:#2e7d32; }
    .badge-nforce { background:#e3f2fd; color:#1565c0; }
    ...
    .modal-section .kv .k { color:#888; font-weight:600; }
    .modal-section .kv .v { color:#333; }
    .empty { text-align:center; padding:3rem; color:#999; }
    .filter-bar { margin-bottom:1rem; display:flex; gap:1rem; align-items:center; }
    .filter-bar input, .filter-bar select { padding:0.4rem 0.75rem; border:1px solid #ddd; border-radius:6px; font-size:0.85rem; }
...
<body>
  <div class="topbar">
    ...
    <div class="user-info">
      Logged in as <strong>${esc(role)}</strong>${distributor ? ` (Distributor: ${esc(distributor)})` : ""}
  ...
  <div class="container">
    ...
    <div class="table-wrap">
      <table id="subsTable">
        ...
        <tbody>
          ${tableRows || '<tr><td colspan="7" class="empty">No submissions found.</td></tr>'}
        </tbody>
      </table>
    </div>
  </div>
  ...
  <script>
    ...
    function filterTable() {
      ...
      document.querySelectorAll('#subsTable tbody tr').forEach(row => {
        ...
        row.style.display = company.includes(q) ? '' : 'none';
      });
    }
    ...
    document.querySelectorAll('.view-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        if (!sub) return;
        document.getElementById('modalTitle').textContent = '📋 ' + (sub.general.companyName || 'Unknown Company');

        function renderSection(title, obj) {
          if (!obj) return '';
          const rows = Object.entries(obj).map(([k,v]) => {
            if (!v) return '';
            return '<div class="k">' + k.replace(/([A-Z])/g,' $1').replace(/^./,c=>c.toUpperCase()) + '</div><div class="v">' + esc(v) + '</div>';
          }).join('');
          return '<div class="modal-section"><h3>' + title + '</h3><div class="kv">' + rows + '</div></div>';
        }

        document.getElementById('modalBody').innerHTML =
          '<div class="modal-section"><div class="kv">' +
          '<div class="k">Submission ID</div><div class="v">' + esc(sub.id) + '</div>' +
          '<div class="k">Submitted At</div><div class="v">' + new Date(sub.submittedAt).toLocaleString() + '</div>' +
          '<div class="k">Distributor</div><div class="v">' + esc(sub.distributor || 'Unassigned') + '</div>' +
          '</div></div>' +
          renderSection('General Company Information', sub.general) +
          renderSection('Application Services', sub.appServices) +
          renderSection('Zero Trust', sub.zeroTrust) +
          renderSection('Developer Platform', sub.developer) +
          renderSection('Additional Information', sub.additional);
    ...
    document.querySelectorAll('.delete-btn').forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        try {
          const resp = await fetch('/admin/api/delete/' + btn.dataset.id, {
            method: 'DELETE',
    ...
    document.querySelectorAll('.distributor-select').forEach(sel => {
      sel.addEventListener('change', async () => {
        const id = sel.dataset.id;
        const dist = sel.value;
        try {
          const resp = await fetch('/admin/api/assign-distributor', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'X-CSRF-Token': CSRF_TOKEN },
            body: JSON.stringify({ id, distributor: dist })
          });
          if (resp.ok) showToast('Distributor updated to ' + (dist || 'Unassigned'), 'success');
